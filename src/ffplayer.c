...
static int player_prepare_or_free(PLAYER *player, int prepare)
{
    ...
    if (ret == MSG_OPEN_DONE) player_play(player);
    return ret;
}
...
static void* av_demux_thread_proc(void *param)
{
    ...
    while (!(player->status & PS_CLOSE)) {
        ...
        if(player->vcodec_context != NULL) {
            if (!((player->vcodec_context->width == 3840 && player->vcodec_context->height == 2160) || (player->vcodec_context->width == 1920 && player->vcodec_context->height == 1080))) {
        ...
        if (retv < 0) {
            ...
            if (player->init_params.auto_reconnect > 0 && av_gettime_relative() - player->read_timelast > player->init_params.auto_reconnect * 1000) {
                ...
                //player_send_message(player->cmnvars.winmsg, MSG_PLAY_COMPLETED, 0);
                pthread_mutex_unlock(&player->lock);
                continue;
            }
            av_usleep(20 * 1000);
...
void player_seek(void *hplayer, int64_t ms, int type)
{
    ...
    if (mainplayer->rear_player) {
        ...
        case SEEK_STEP_FORWARD:
            render_pause(rear_player->render, 1);
            render_setparam(rear_player->render, PARAM_RENDER_STEPFORWARD, NULL);
            return;
        case SEEK_STEP_BACKWARD:
            rear_player->seek_dest = av_rescale_q(rear_player->seek_vpts, rear_player->vstream_timebase, TIMEBASE_MS) - 1000 * rear_player->vfrate.den / rear_player->vfrate.num - 1;
            rear_player->seek_pos = rear_player->seek_vpts + av_rescale_q(ms, TIMEBASE_MS, rear_player->vstream_timebase);
            rear_player->seek_diff = 0;
            rear_player->seek_sidx = rear_player->vstream_index;
            pthread_mutex_lock(&rear_player->lock);
            rear_player->status |= PS_R_PAUSE;
            pthread_mutex_unlock(&rear_player->lock);
        default:
            ...
            rear_player->seek_diff = 100; // Play position event (in 1 second, callback) provided
    ...
    switch (type) {
        ...
        default:
            ...
            front_player->seek_diff = 100; // Play position event (in 1 second, callback) provided
...
void player_getparam(void *hplayer, int id, void *param)
{
    ...
    case PARAM_DATARATE_VALUE:
        ...
        break;
    case PARAM_WHICH_VIDEO_FISHIED:
        // Compare the durations of two video files
        videoflag = player->avformat_context->duration > rear_player->avformat_context->duration ? 0 : 1;
        // Assign the value of videoflag to the memory location pointed to by param
        *(int*)param = videoflag;
        break;
    default:
