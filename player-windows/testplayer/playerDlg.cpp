...
BOOL CplayerDlg::PreTranslateMessage(MSG *pMsg) 
{
    ...
    if (pMsg->message == MSG_FANPLAYER) {
        ...
        case MSG_NOT_4K:
            OnStop();
            AfxMessageBox(_T("This player supports only 4K and FHD. Please check this video again"), MB_OK);
            break;
        case MSG_GET_FPS:
            float frame_rate;
        ...
        case MSG_PLAY_COMPLETED:
            if (!m_bLiveStream) {
                ...
                if (m_bPlayerMode == true) {
                    m_nPlayerStatus = 0;
                    OnStop();
                    AfxMessageBox(_T("MSG_PLAY_COMPLETED"), MB_OK);
                }
                else {
                    int videoflag;
                    player_getparam(m_ffPlayer, PARAM_WHICH_VIDEO_FISHIED, &videoflag);
                    OnStop();
                    m_nPlayerStatus = 0;
                    if (videoflag == 0) {
                        AfxMessageBox(_T("MSG_FRONTVIDEO_PLAY_COMPLETED"), MB_OK);
                    }
                    else {
                        AfxMessageBox(_T("MSG_REARVIDEO_PLAY_COMPLETED"), MB_OK);
                    }
                }
            }
            break;
...
void CplayerDlg::OnStepForward()
{
    ...
    KillTimer(TIMER_ID_PROGRESS);
    player_getparam(m_ffPlayer, PARAM_MEDIA_POSITION, &m_llLastPos);
    player_seek(m_ffPlayer, m_llLastPos + 10000, 0);
    SetTimer(TIMER_ID_PROGRESS, 100, NULL);

    if (pauseFlag) {
        player_pause(m_ffPlayer);
        m_bPlayPause = pauseFlag;
    }
...
void CplayerDlg::OnStepBackward()
{
    ...
    KillTimer(TIMER_ID_PROGRESS);
    player_getparam(m_ffPlayer, PARAM_MEDIA_POSITION, &m_llLastPos);
    player_seek(m_ffPlayer, m_llLastPos - 10000 < 0 ? 1 : m_llLastPos - 10000, 0);
    SetTimer(TIMER_ID_PROGRESS, 100, NULL);
...
void CplayerDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (m_bLiveDeskMode) {
        ...
        if (point.y > m_rtClient.bottom - 8) {
            LONGLONG total = 1;
            player_seek(m_ffPlayer, m_llLastPos, 1);
            player_getparam(m_ffPlayer, PARAM_MEDIA_DURATION, &total);
            ...
            if (m_bPlayerMode == false) {
                ...
                player_seek(m_ffPlayer, m_llLastPos + 1, 0);
                SetTimer(TIMER_ID_PROGRESS, 100, NULL);
                if (m_bPlayPause) {
                    Sleep(100);
                    player_pause(m_ffPlayer);
...
void CplayerDlg::OnVolumeUp()
{
    ...
    volume += 5; if (volume > 73) volume = 73;
    player_setparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);

    _stprintf(m_strTxt, TEXT("Volume: %d"), (int)((volume+182)*100) / 255);
    PlayerShowText(2000);
}
...
void CplayerDlg::OnVolumeDown()
{
    ...
    int volume;
    player_getparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);
    volume -= 5; if (volume < -182) volume = -182;
    player_setparam(m_ffPlayer, PARAM_AUDIO_VOLUME, &volume);
...
void CplayerDlg::OnGetFps()
{
    ...
    // Display the frame_rate variable in a message box
    CString message;
    message.Format(_T("Video frame rate is %s"), frame_rate_str);
    AfxMessageBox(message, MB_OK);
}
